...
def calculate_metrics_by_period(data: pd.DataFrame, positions: np.ndarray, returns: np.ndarray, nav: np.ndarray) -> dict:
    """计算给定时期的评价指标

    Args:
        data (pd.DataFrame): 原始数据
        positions (np.ndarray): 持仓信息
        returns (np.ndarray): 收益率
        nav (np.ndarray): 净值
    Returns:
        dict: 评价指标计算结果
    """
...
def calculate_returns_and_nav(data: pd.DataFrame, positions: np.ndarray) -> tuple:
    """计算收益率和净值

    Args:
        data (pd.DataFrame): 数据
        positions (np.ndarray): 持仓信号
    Returns:
        tuple: (list, numpy.ndarray) 每次预测的收益率(去掉了最后一个bar)，累积净值
    """
    price_change = data["close"].pct_change().fillna(0)
    strategy_returns = positions[:-1] * price_change[1:] # 每一个position都是预测值，要与下一个bar的price_change相乘
...
def calculate_positions(prediction: np.ndarray, scale: float = 0.0005, position_size: float = 1) -> np.ndarray:
    """计算持仓信号 
       注：持仓信号可以用sigmoid  tanh, relu, zscore, 等函数先进行计算处理
    Args:
        prediction (np.ndarray): 预测值
        scale (float, optional): 缩放因子. Defaults to 0.0005.
        position_size (int, optional): 持仓规模. Defaults to 1.
    Returns:
        np.ndarray: 持仓信号
    """
    position = (prediction / scale) * position_size
    return np.clip(position, -1, 1)

def backtest(original_data: pd.DataFrame, index_code: str, frequency: int, n_days: int) -> pd.DataFrame:
    """回测核心函数

    Args:
        original_data (pd.DataFrame): 原始数据 对齐了因子
        index_code (str): 指数代码
        frequency (int): 频率
        n_days (int): 预测步长
    """
    # 1. 数据预处理
    final_frame = original_data[["etime", "tdate", "close", "fct"]].dropna(axis=0).reset_index(drop=True)
    ...
    final_frame["ret"] = final_frame["close"].shift(-t_delta) / final_frame["close"] - 1
    final_frame = final_frame.dropna(axis=0).reset_index(drop=True)

    # 3. 训练集和测试集划分 !!!这里改成动态分配！！！！
    train_set_end_index = final_frame[
        (final_frame["etime"].dt.year == 2019) &
        (final_frame["etime"].dt.month == 12) &
        (final_frame["etime"].dt.day == 31)
        ].index[0]
    ...
    indicators_frame.loc['总体'] = calculate_metrics_by_period(final_frame, total_positions, total_returns, total_nav)
